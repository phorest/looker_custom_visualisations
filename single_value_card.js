looker.plugins.visualizations.add({
  ...
  create: function(element, config) {
    // Initial setup - we insert the container here.
    // If Looker clears 'element' later, updateAsync will handle it.
    this._addBaseStructure(element);
  },
  
  // Helper to add the style and container
  _addBaseStructure: function(element) {
    element.innerHTML = `
      ...
      <div class="vis-container" style="padding:0; width:100%; height:100%; box-sizing:border-box; background:#f8f9fa; display: flex; align-items: flex-start;">
      </div>
    `;
  },
  ...
  updateAsync: function(data, element, config, queryResponse, details, done) {
    ...
    // --- SAFETY CHECK: REBUILD CONTAINER IF MISSING ---
    // If Looker cleared the element (common on resize), re-inject the base structure.
